...
def test_composition_failure_releases_all_serving_ownership(tmp_path, monkeypatch):
    ...
    with TestClient(reopened, base_url=profile.http_origin) as client:
        ...
        assert composition.route_count == 146
        assert composition.contribution_ids == ("core-v1", "extension-candidates-v1", "run-approvals-v1",
            "works-v1", "run-consents-v1", "claude-connection-v1", "backups-v1", "retention-v1", "credentials-v1", "work-models-v1", "source-readings-v1", "conversations-v1", "budget-policies-v1", "runs-v1", "artifact-index-v1", "run-trace-v1", "run-feedback-v1", "graphs-v1", "design-workspace-v1", "hypotheses-v1", "inquiries-v1", "versions-v1", "deployment-prepare-v1", "provider-conformance-v1", "provider-installation-v1", "extension-bindings-v1", "browser-grants-v1", "platform-update-v1",
            "service-clients-v1")
        assert composition.route_ids == ("events.read", "events.stream", "events.type", "snapshot.read",
            "commands.read", "commands.create", "extensions.candidates.create", "extensions.candidates.list", "extensions.candidates.read",
            ...
            "run_consents.record", "run_consents.read", "run_consents.revoke",
            "connections.claude.read", "connections.claude.key", "connections.claude.forget",
            "connections.claude.catalog", "connections.claude.model_choice",
            ...
            "credentials.catalog_refresh", "credentials.model_choice",
            "work_models.draft", "work_models.read", "work_models.confirm",
            "source_readings.list", "source_readings.read", "source_readings.latest",
            "conversations.read", "conversations.message", "conversations.propose",
            "conversations.challenge", "conversations.approve",
            "budget_policies.list", "budget_policies.create",
            ...
            "runs.alternative_difference_observe", "runs.environments", "artifacts.index", "runs.trace", "runs.feedback", "runs.feedback_record",
            "graphs.read", "design_requests.list", "design_requests.create", "design_requests.read", "design_requests.derive",
            "design_requests.review", "design_requests.prepare", "design_requests.generate", "design_requests.cancel", "hypotheses.read", "hypotheses.propose", "inquiries.read", "inquiries.open",
            "inquiries.answer", "inquiries.evidence", "inquiries.judge", "inquiries.audit",
            "versions.read", "versions.adopt", "versions.decide", "versions.activate", "versions.rollback",
            "versions.tool_effect_boundaries.read", "versions.tool_effect_boundaries.decide",
            ...
            "deployment.provider-requests.read", "deployment.provider-requests.receipts",
            "deployment.provider-requests.consume", "extensions.provider-conformance.execute",
            "extensions.provider-conformance.read",
            "extensions.provider-transport-qualification.read",
            "extensions.provider-transport-qualification.execute", "extensions.provider-installation.execute",
            "extensions.provider-installation.read", "extensions.installations.list",
            "extensions.bindings.list", "extensions.bindings.read", "extensions.bindings.slot_key",
            "extensions.bindings.bind", "extensions.bindings.disable", "extensions.bindings.rollback",
            "extensions.bindings.retention_release", "browser_grants.read", "browser_grants.create",
            "browser_grants.revoke", "platform.update.read",
            "service-clients.list", "service-clients.create", "service-clients.read",
            "service-clients.rotate", "service-clients.revoke")
        assert len(set(composition.route_ids)) == 146
