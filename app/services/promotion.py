...
def _stamp(value, label):
    if type(value) is not str or len(value) > 64:
        raise PromotionError(f"{label} must be a UTC timestamp")
    try:
        datetime.strptime(value, "%Y-%m-%dT%H:%M:%S.%fZ")  # noqa: DTZ007
    except ValueError as exc:
        raise PromotionError(f"{label} must be a UTC timestamp") from exc
    return value
...
def record_promotion_decision(value) -> PromotionDecision:
    ...
    if report.mode != "sealed_offline" or report.status != "passed":
        # Observation-only, limited, failed or invalid evidence never backs
        # an operational promotion decision (G-11).
        raise PromotionError(
            "promotion requires a passed sealed-offline validation report"
        )
    ...
    return _issue(
        ...
        validation_report=EntityRef(
            ...
            bundle_ref.id,
            bundle_ref.version,
            sha256(canonical_json(report.as_dict())).hexdigest(),
        ),
...
def rollback_environment(state, reason) -> PromotionState:
    ...
    previous, _lifecycle = state.history[-1]
    return _issue(
        PromotionState,
        ...
        history=(
            *state.history[:-1],
            (previous, "active"),
            (state.current_environment, "rolled_back"),
        ),
