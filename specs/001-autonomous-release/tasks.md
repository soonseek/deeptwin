...
- [ ] T048 [US3] Connect live graph/role visits/attempts/inputs/outputs/tools/cancel/recovery to common UI in app/static/runtime.mjs and app/api/routes.py; keep past attempts distinct (UX-AC04/10). 2026-09-18 slice: the route layer landed — `runs-v1` (`POST /api/v1/runs`, `GET|HEAD /api/v1/runs/{run_id}`, `POST …/resume`) over `PersistentRuns` with the injected code-owned run executor (evidence/runs-routes-browser-path.md); the logic half of `app/static/runtime.mjs` landed (evidence/runtime-gui-logic-t048.md; pure view/observer over the routes, no DOM wiring, the shell's `api` helper still sends the wrong CSRF header); the DOM half, cancel/recovery routes and the attempt layer remain open.
...
- [ ] T055 [US5] Implement format-aware differences/trace slicing and system/expert/exception/error/no-generalization hypotheses in app/services/diagnosis.py and app/tests/test_diagnosis.py; no unsupported single-cause claim (FR-018). Recorded observation contracts exist; trace read-back and boundary slicing over real scheduler runs now exist (evidence/run-trace-t055.md); format-aware artifact differences over sliced traces remain open.
