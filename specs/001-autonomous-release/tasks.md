...
- [ ] T048 [US3] Connect live graph/role visits/attempts/inputs/outputs/tools/cancel/recovery to common UI in app/static/runtime.mjs and app/api/routes.py; keep past attempts distinct (UX-AC04/10). 2026-09-18 slice: the route layer landed — `runs-v1` (`POST /api/v1/runs`, `GET|HEAD /api/v1/runs/{run_id}`, `POST …/resume`) over `PersistentRuns` with the injected code-owned run executor (evidence/runs-routes-browser-path.md); `app/static/runtime.mjs`, cancel/recovery routes and the attempt layer remain open.
