...
- [ ] T087 Implement ADR-014's versioned semantic extension framework in app/extensions/, the domain 2026-09-18 slice: the worker's code-owned operation registry now carries `status` (an actual metadata reading) behind the closed `extension-execute-v1` grammar, with the control-side transport sealing the output (evidence/worker-execute-transport-t087.md); `describe_tools` landed the same day over the worker's empty code-owned tool table (evidence/worker-describe-tools-t087.md), and the artifact input leg of the execute exchange landed over T018's bounded stream (evidence/worker-artifact-input-leg-t018-t087.md: declared inputs, profile-gated admission, no registered consumer yet); the first real tool `text_profile` and `invoke_tool` landed over the leg (evidence/worker-text-profile-tool-t087.md: control mirrors the table, verifies the reply's tool, digest, size and usage); the reverse leg landed with the second tool `text_normalize` (evidence/worker-output-artifacts-t018-t087.md: output artifacts offered before the reply, admitted under the mirrored output contract, verified, imported as registered content and sealed); the `ToolCall` record and the effect gate landed (evidence/tool-call-record-effect-gate-t087.md: a ledger row per attempt, write-ahead intent, settled from what control observed, the approval recorded for external effects); the `output_bytes` reservation from the tool's bound landed (evidence/output-bytes-reservation-t087.md: the transport states and enforces its output bound, the dispatcher refuses a binding under it); the approval's verification landed (evidence/tool-call-approval-verification-t087.md: the recorded decision for the run, node and tool scope, exact record, before the send); `cancel`, tool arguments, the vouched dispatch effect is journaled by the dispatcher (evidence/vouched-transport-effect-journal.md; a free retry after a committed send intent stays open as a ledger trust-model change); the tool boundary's effect vocabulary is the ports contract's (evidence/tool-effect-vocabulary-t047-t087.md; the `tool.requested` event's own vocabulary now is the ports contract's seven classes too — 2026-09-23, pinned by test_domain_events, event-metadata export regenerated); the ToolDefinition-backed gate landed (evidence/tooldefinition-effect-gate-t087.md: the authority's definition carries the class, a bound external tool requires its derived scope through a human gate, the transport refuses a disagreeing mirror); per-execution approval binding, a production graph that binds a tool gate, the ports contract's per-tool input count/role binding and every other port remain open. 2026-09-25 slice: the per-execution approval binding and the per-tool input declaration landed (evidence/tool-execution-binding-t087-2026-09-25.md: a `run-approval-v2` decision binds the gate's run/node/scope plus the execution id, executing node and attempt number, v1 stays readable and is refused for dispatch; the transport verifies the exact attempt before the channel and the ledger admits one approval for one attempt's call; each tool declares a `ToolArtifactInputContractV1` checked by the dispatcher at build and before the connection and re-checked by the worker); a production graph that binds a tool gate, the scheduler consuming v2 decisions, an HTTP route for v2 decisions, the atomic approval-use/budget/send claim, the compiled ToolDefinition carrying the input declaration, selectors on the wire and every other port remain open. 2026-09-25 (later): the owner route for v2 decisions landed (evidence/execution-approval-route-t087-2026-09-25.md: the ledger records its own ask per attempt — gate, execution, attempt, the executing node from its execution row, the inputs digest; `GET|POST /api/v1/runs/{run}/approvals/executions` lists the asks and records the owner's decision only when every field equals the ask, replay-safe, conflicting on change; the transport refuses a decision that does not answer this attempt's ask or its inputs digest; `approvals.mjs` shows the exact execution/attempt); no production caller of the ask, the scheduler consuming v2, the atomic claim, expiry and tool arguments remain open. 2026-09-25 (approval screen): the observe page renders the v2 asks from `GET …/approvals/executions` with approve/reject posted to `POST …/approvals/executions`, a retry attempt marked as needing its own decision and a superseded one as authorizing nothing (evidence/approval-screen-2026-09-25.md; unit-tested only — no supported path produces a v2 ask for a browser case yet). 2026-09-25 (scheduler/gate passage): the scheduler consumes v2 decisions per attempt and the claim is atomic (evidence/tool-gate-scheduler-t087-2026-09-25.md: a gated external-tool node's gate scope is no longer a v1 scope; before the handler the scheduler records the ledger's ask for exactly the attempt the dispatcher would send next, with the transport's inputs digest — the first production caller of `request_execution_approval` — and pauses (`awaiting_execution`) until the owner's decision for that attempt; rejected, expired or recovery-superseded stops the visit with a recorded reason (`refuse_execution_approval`); a retry is a new ask; approval use, ToolCall intent, budget reservation and send intent commit in one ledger transaction (`ToolDispatchClaim`), a crash between claim and send is reconciled with no send; asks and v2 decisions carry a server-set bounded expiry refused at the claim and listed/shown `expired`; a real-browser E2E with a test-registered test-actor tool approves, rejects and retries through the approval screen). Still open: no production graph or worker tool binds a tool gate (the path is driven only with the test-actor tool), re-asking after an expiry, tool arguments/selectors, and every other port. 2026-09-25 (provider-transport manifest): the provider port's credentialed transport is expressed as a `provider-transport-manifest-v1` data manifest and qualified through the existing installation/conformance path — a matched verified-installation `provider-conformance-command-v2` run (still current) plus an offline transport conformance of the manifest's request surface against a local mock provider over the same four fixed vectors — sealed as a `validation_report` and published to the credential gateway, which serves sends only through the qualified manifest digest (evidence/credential-vault-t090.md §2026-09-25 manifest and budget). 2026-09-25 (later): the owner path for the qualification act landed (`GET|POST /api/v1/extensions/provider-transport-qualification`, credentials-section UI; evidence/credential-vault-t090.md §2026-09-25 qualification owner path); other ports unchanged. 2026-09-25 (Settings > Extensions): a first `Settings > Extensions` screen landed (`app/static/extensions.mjs` at `settings.html#settings-extensions`, hub entry `확장`; evidence/extensions-ui-2026-09-25.md). It reads and acts only through the existing candidate, provider-installation, provider-conformance and transport-qualification routes: candidate inspection (source/license/kind/port/digests/platforms) and code-free metadata registration, release-evidence packet verification sent byte-for-byte, and a verified-installation conformance run carrying exactly the staged/verified head read, with 409 shown as a conflict. Items no route supplies (bindings, five-field slot key+digest, slot/selector, bind/disable/rollback, history, rollback retention and release, affected environments, trust tier, deployment-request status) are shown as not supplied; staging is a handoff description only. Unit-tested, plus a real-browser case of the reads, registration and conflict, and the refused packet; the verified-installation success path is not driven in a browser (pytest-only fixtures). The binding/retention/qualification/retirement/deployment and code-free-definition routes and their UI remain open. 2026-09-25 (extension bindings): the `extension-bindings-v1` owner routes landed with durable binding history/head and rollback retention (evidence/extensions-ui-2026-09-25.md §bindings): `extension_binding` records keyed by the exact five-field `BindingSlotKeyV1` digest (revision = record version, head = newest), bind/supersede/disable/rollback with exact expected-head CAS (409 `binding_head_stale`, no write), same-slot competition only through that CAS and coexistence of other slots/selectors, a `retained` retention head for every displaced active revision, rollback that re-checks the target's qualification and consumes its retention, the owner-only `ReleaseExtensionRollbackRetention` with the exact closed command/result of contracts/api.md and a server warning, `extension.binding_changed` in the same transaction, replay-safe command ids; `POST …/binding-slot-keys` returns the key and digest the server computes; bounded list pages for candidates (`extensions.candidates.list`), installations (trust tier from the port contract, service tuple, staging request/receipt refs) and binding slots (128 installed routes). The only qualification record is the sealed provider-transport qualification, so only `provider-port-v1` is bindable and only the provider selector family is admitted; `target_scope_fingerprint` is this slice's own derivation (canonical digest of the closed scope) because the contracts fix the field, not its derivation; the contract's `/extensions/{id}/bindings…` paths are served without the `{id}` segment (it would collide with the fixed extension segments). Settings > Extensions shows and acts over these routes. Still open: durable `ExtensionQualification` records (five checks, expiry, context heads) for any port, other selector families, the contract's distinct binding/retention event names (`extension.rollback_retention_*` and superseded/disabled/rolled_back are not registered; release emits no event), environment versions binding binding revisions (affected environments list is always empty), startup reconciliation of expiry/revoked grants before dispatch, the dispatch path reading these heads, retirement/uninstall/replace and the deployment-request projection, and code-free definition import. 2026-09-26 (dispatch heads): the provider semantic dispatch resolves its binding from the durable slot head at every load (`app/extensions/binding_heads.py`; disabled, superseded, rolled-back-away or absent heads refused before any send, a head moved between the send's build and its commit refused by the pre-commit re-load; the semantic authority can no longer carry a binding record, only grant/handle refs); environment versions record the binding revisions they were prepared with (`environment-version-v2`/`environment-record-v2`) and the slot inspection lists them with re-preparation state (a binding change marks, never re-prepares or promotes); `extension.binding_activated|superseded|disabled|rolled_back` and `extension.rollback_retention_created|released|consumed` are registered and committed in the binding transaction; startup reconciles heads against retention, command and event records and the dispatch resolver refuses an inconsistent slot (evidence/extensions-ui-2026-09-25.md §2026-09-26). Still open: durable `ExtensionQualification` records for any port, grant/handle refs on the binding revision, expiry/revoked-grant reconciliation, retirement/uninstall/replace, other selector families and ports. 2026-09-26 (package roots): `sdk/python/deeptwin_ext/` and `sdk/python/deeptwin_client/` are separate PEP 517 package roots (`deeptwin-ext`/`deeptwin-client` 0.1.0, own metadata and versions, Apache-2.0, no dependencies) built offline by a stdlib in-tree backend into exact, byte-reproducible wheel and sdist artifacts; each artifact is pip-installed (`--no-index --no-deps`) into a fresh venv and exercised with `python -I` outside the repository where `import app` fails; the installed client reaches the real `create_app` portable-HTTPS server over verified TLS, where the owner path gets 200 on the same mounted routes and the client's bearer gets the uniform 401 because no route admits a bearer (evidence/sdk-client-packages-t087-2026-09-26.md; test_sdk_package_builds, test_sdk_package_install, test_extension_client_blackbox). Still open: T025 mounting the bearer path (every contribution is `browser_session`), then admitted client calls and browser/client parity across restart/concurrency; a generated OpenAPI document; `deeptwin_ext` port-schema bindings/refinements; `extension_routes.py`/`extensions-v1.json`; the OCI tool fixture.
  ...
  (2026-09-26: the two package roots, their wheel/sdist artifacts and fresh-venv installs landed, and the installed client reaches the real TLS server's mounted routes; no route admits a bearer yet, so parity stays open — evidence/sdk-client-packages-t087-2026-09-26.md.)
  (2026-09-26, bearer reads: T025 mounted the bearer path, so the installed client's calls are now admitted — the owner creates scoped service clients in a TLS browser session and the installed `deeptwin-client`, in its own venv unable to `import app`, reads `/api/v1/snapshot` (`snapshot.read`) and `/api/v1/events` (`events.read`) with the same projection and cursor the browser reads; a wrong-scope credential gets 403, browser-only routes (the extension reads among them) 401, a revoked secret 401, plain `http://` and an untrusted chain are refused, and no secret reaches any output (test_extension_client_blackbox; evidence/bearer-service-clients-2026-09-26.md). Still open: command receipt/revision/event-order parity across restart and concurrency (no bearer command route: the contracts name no command scope — open owner decision), extension-read scopes with T087's own `extensions-v1.json`, OpenAPI, the `deeptwin_ext` port-schema bindings and the OCI tool fixture.)
...
- [ ] T023 [US1] 2026-09-18 slice: the intake's server surface on the supported factory landed — `works-v1` (`POST /api/v1/works`, `GET|HEAD …/{id}`, `POST …/{id}/revisions`) sealing immutable `work_revision` records under owner commands (evidence/works-routes-intake-t023.md); 2026-09-19: the first work screen landed over it (`work.html`/`work.mjs`, evidence/intake-page-t023.md: the §5.1 notices, the prompt, a browser-only draft said as such, idempotent saves settled in order, the draft's base revision, conflict with reopen or rebase); sources/files, the microphone, understanding requests and provider/key/model GUI stay open. 2026-09-25 (evidence/t023-audit-2026-09-25.md, requirement-by-requirement table): on the supported factory the owner now reads each retained original only on command (`source-readings-v1`: complete/partial/unreadable with named reasons, PDF/DOCX only through the isolated document worker, reader absent → nothing sealed), the work-model understanding request carries exactly the readings made, the shared conversation (`conversations-v1`, `PersistentConversation` in app/services/conversation.py, `createWorkConversation` in app/static/chat.mjs) records messages bound to the current revision and exact same-work references and runs referenced-object commands (work model confirm/reject) only through a session-bound one-time challenge answered by button or exact phrase — ambiguous/spoofed/foreign-session/expired/stale answers refused with nothing changed (app/tests/test_conversation.py), the owner authors run budgets (`budget-policies-v1`, records page), and a guard keeps every served asset free of the loopback device/native-app wording; routes 104 → 114. Still open, so unticked: provider mutation closure with T025/T090 (the credential pipeline lives at T090's paths), binding-scoped invalidation of already sealed model choices, managed-runner state (T088), and the microphone (T024). 2026-09-25 follow-up: reading text is now erased with a deleted original — `source-reading-v2` keeps it as its own blob, the T073 deletion preview lists every reading (and the work-model drafts made from them, which are kept), consent tombstones the text per content address, readers answer `deleted`/410, later backups lack it (evidence/t023-audit-2026-09-25.md follow-up; evidence/source-deletion-t073-2026-09-23.md 2026-09-25). Add provider/key/model/budget GUI and same-space first-input recovery in
  ...
  2026-09-26 note (UI phase 5, not a tick): the work page became a step flow — ① 설명·자료, ② 업무 이해,
  ③ 환경 제안, ④ 준비·시작 (`app/static/work-steps.mjs`): only reached steps and the one next step are drawn, each
  reached by what the server holds (a work model of this work, a design request of this work, a prepared environment),
  with one next action named per UX-D06 and a one-sentence reason with the raw server reason under "기술 정보";
  complete steps fold to a reopenable one-line summary; the work model reopens after a reload from this work's
  `understanding.completed` events; export and source deletion moved to a work menu (US7-3); the side panel lists
  this work's runs; a work switcher and "새 업무" appear when several works are saved (listed from `work.created`
  events, since the public snapshot does not carry `works-v1` works) and refuse to switch over unsaved input.
  No route added (docs/ui/2026-09-26-product-ux-redesign.md §14). The items above stay open.
- [ ] T090 [US1] Implement `CredentialedProviderTransport` and its dedicated gateway in
  ...
  2026-09-25 (open, T087 manifest + budget binding): the gateway's send path now binds only
  through a provider-transport manifest (`provider-transport-manifest-v1`, canonical data at
  app/workers/transport_manifests/claude-api-v1.json): origin, methods, path prefixes, projected
  header values, auth header, byte bounds and each endpoint's method/path/query/cursor/media come
  from the manifest, and the vault delivers custody only while its adopted
  `provider-transport-qualification-v1` (`bind_transport`, revision-monotone) names that
  manifest's digest; an unqualified or changed manifest refuses before any provider byte. The
  qualification act (`PersistentTransportQualification`, service export only) requires a matched
  4/4 verified-installation conformance run whose admission is still current plus an offline
  transport conformance of the manifest against a local mock provider, seals a record and
  publishes the document. Every send names a budget reservation: messages the runtime ledger's,
  model pages a write-ahead reservation in the budget book (zero-cost, bounded catalog policy;
  reserved and dispatched in one transaction, settled with observed usage; a crashed page is
  settled unknown and never re-sent); the gateway journals each consumed reservation and refuses
  reuse across restarts (evidence/credential-vault-t090.md §2026-09-25 manifest and budget).
  Still open: the loopback-substituted origin is the
  only part of the manifest conformance does not exercise, consumed-reservation rows are capped
  (16,384) with no pruning, message sends' budget settlement remains the ledger's (runs still use
  the direct adapter), plus the items above.
  2026-09-25 (later, open, qualification owner path): `GET|POST
  /api/v1/extensions/provider-transport-qualification` (provider-conformance-v1 contribution,
  119 installed routes) reads the manifest digest, requirements, prerequisite (naming the run that
  meets it or which one is missing), the newest sealed qualification and the gateway's adopted
  revision (new nonsecret `transports` gateway read), and runs the qualification act (owner, CSRF,
  command id replay-safe via a sealed command record, exact refusal reasons) publishing through
  `bind_transport`; the credentials section shows it with a "전송 매니페스트 검증" button
  (evidence/credential-vault-t090.md §2026-09-25 qualification owner path). Still open: a
  production owner path to stage a verified installation and run its conformance end to end in a
  browser case (the browser case shows the real refusal and uses a fixture adoption for the
  catalog refresh), plus the items above.
...
- [ ] T025 [US1] Implement web first-owner/auth/deployment-authority foundations in
  ...
  2026-09-25 (open, maintenance tool + recovery screen): `python -m app.operations.deployment_control {status,prepare,cancel,import}` (app/operations/deployment_control.py, operator tooling only) runs with the control plane stopped (serving lock; `busy` otherwise): prepare seals the `owner_recovery` request with a fresh nonce from the root/config/DB at the same epoch N, bound to the new capability's non-secret verifier; import verifies the externally signed receipt against the recovery trust set, binds the verifier, journals `importing`, advances the session root to N+1 and rewrites the deployment configuration (temp+fsync+rename); cancel ends a prepared request (its receipt is then refused), an import in progress cannot be cancelled; re-runs are idempotent and a crash between any two steps completes on re-run. `/health` adds `recovered: true` after a recovery and start.mjs shows the honest recovered re-setup (and a no-form reconciling state). Tests: test_deployment_control_tool.py (round trip into a real reconciling start, busy, cancel, wrong/forged/foreign/replayed receipts, crash at each step), start.test.mjs, and the real-browser browser-owner-recovery-t025.test.mjs (tool + restart + re-setup with the new capability; old session/capability/password refused) — evidence/owner-recovery-reconcile-t025-2026-09-25.md §later. Stays open: a real instance-operator signing adapter (signing exists only in tests), a real deployment recovery, a separate lifecycle/consumption CAS and request/receipt channels, CredentialVault/credential_client; the recovery design file stays "proposal only" (no approval record in the repo beyond a test docstring). 2026-09-26: the owner approved the recovery design (decisions.md); the other items above stay open.
  2026-09-26 (open, service clients + bearer reads): the common service-client path is mounted on the supported factory through the frozen seam (evidence/bearer-service-clients-2026-09-26.md). `service-clients-v1` (`app/api/service_clients.py`, catalog entry, descriptor) gives the owner create (secret once, digest-only storage)/list/read/rotate/revoke as browser-session commands with CSRF; creation is refused unless the instance is portable HTTPS, the grant names exactly that profile, expiry ≤24h, and every scope is one a composed route declares for a bearer. A descriptor policy `browser_session_or_service_bearer` marks the only bearer routes: the public snapshot (`snapshot.read`) and event reads (`events.read`, including SSE) in `core-v1`. `WebBoundary` parses a bearer only on the portable HTTPS profile over TLS, with no cookie, on a declared route (everything else: uniform 401 before parsing), runs `ServiceClientAuthenticator` (single header, source/route/client buckets, durable last-used CAS, 403 on scope, 429 on capacity) and hands the handlers a read-only `ServiceClientRead` that is re-verified after the read, so a revoke takes effect at once. Routes 138 → 143. Tests: test_service_client_bearer.py (composed factory, both profiles, cold restart, rotation, revocation during a read, rate buckets, no secret in storage/logs) and the installed-client black box. Open owner decisions (decisions.md): which further routes and scope names a bearer may reach (commands, artifacts, extension reads) and the rate-bucket numbers. Stays open: bearer commands and their receipt parity, a Settings UI for service clients, the recovery items above, CredentialVault/credential_client.
  2026-09-26 (open, UI phase 6): `설정 > 서비스 클라이언트` (app/static/service-clients.mjs) lists, creates (the grantable scopes only, an expiry under 24 hours), rotates and revokes (each after a confirmation) the owner's clients over the unchanged `service-clients-v1` routes; the one-time secret is shown once with a copy button and a warning and is never written to browser storage; a loopback instance's refusal is said in words with the server's answer under 기술 정보 (service-clients.test.mjs, browser-service-clients.test.mjs over the portable HTTPS fixture, browser-a11y.test.mjs; docs/ui/2026-09-26-product-ux-redesign.md §15). Stays open: bearer commands and their receipt parity, the recovery items above, CredentialVault/credential_client.
...
- [x] T038 [US2] Exercise real generation→critique→selection and 0/1/2/3 valid-candidate/revision/cancel paths in app/tests/browser-design.test.mjs and specs/001-autonomous-release/evidence/us2.md; no fixture scores presented as live (SC-003/SC-005).
  ...
  run, refused calls recorded as refusals, cancel before the next model call; an edit re-reviewed
  through a revision generation call), with the workspace's `후보 생성` UI. In real Chrome
  (`app/tests/browser-design.test.mjs`, over `app/tests/fixtures/design_arc_server.py`): 0/1/2/3
  presented with the real count and reasons, a refused call, edit → re-review → prepare, merge still
  unavailable, cancel — scripted TEST-ACTOR turns and a SIMULATED critic qualification, labelled on
  the page, never presented as live and never a release qualification. ONE live run
  (`test_claude_live_design_arc.py`, USD ≈ 0.19 of the 3.00 cap): a real candidate accepted, the
...
- [ ] T048 [US3] Connect live graph/role visits/attempts/inputs/outputs/tools/cancel/recovery to common UI in app/static/runtime.mjs and app/api/routes.py; keep past attempts distinct (UX-AC04/10). 2026-09-18 slice: the route layer landed — `runs-v1` (`POST /api/v1/runs`, `GET|HEAD /api/v1/runs/{run_id}`, `POST …/resume`) over `PersistentRuns` with the injected code-owned run executor (evidence/runs-routes-browser-path.md); the logic half of `app/static/runtime.mjs` landed (evidence/runtime-gui-logic-t048.md) and the supported factory now serves every shell module with the shell on `X-DeepTwin-CSRF` (evidence/shell-assets-supported-factory.md); the cancel and recovery routes landed (`runs.cancel`, `runs.recover`; evidence/run-cancel-route.md, evidence/run-recover-route.md); the attempt layer in the run trace landed (`TraceAttempt`, `producing_attempt_id`; evidence/run-trace-attempt-layer-t048.md: past attempts distinct, the result attributed only to the attempt the bound row names); the supported session client landed (`app/static/session.mjs`, evidence/shell-session-client-supported.md: base path from the location, the token from `GET {base}session`, the observer's request adapter); the run view's DOM half landed (`app/static/run-panel.mjs`, evidence/run-panel-dom-t048.md: rows per node and per past attempt, commands gated by the server's admission, refusals re-read and retained); the run source landed as the public snapshot's run list (`app/static/run-list.mjs`, evidence/run-list-source-t048.md; a creation surface waits on the consent/design line: no production path records a run consent, environment or work revision); the mount landed as the public observation page on the supported factory (`app/static/observe.html`/`observe.mjs`, evidence/observe-page-mount-t048.md: the owner session established from the cookie, the list and panel mounted, honest text without one); the login UI landed (T025, evidence/first-screen-setup-login-t025.md); the intake's server surface landed (`works-v1`, evidence/works-routes-intake-t023.md) and the intake page over it (evidence/intake-page-t023.md); the owner's run consent landed (`run-consents-v1`, evidence/run-consent-record-t048.md: one immutable `run_consent` per command over the exact graph, work revision, environment and budget policy records, the owner's `approval.decided` in the same transaction, replay/conflict, the run route accepting it); the run route verifies that its consent names exactly its inputs and spends it on one run (evidence/run-consent-verification-t048.md); the `environment` record has its producer (evidence/environment-record-t048.md: one record per prepared version, cut from its own head); consent revocation and expiry landed 2026-09-23 (evidence/run-consent-revocation-t048-2026-09-23.md: a revoked or expired consent starts no run, and resume/recover/replay under it refuse); the design arc's own production path, run creation from the intake page and T049's browser case stay open. 2026-09-25: the observe page's approval screen landed (`app/static/approval-screen.mjs`, evidence/approval-screen-2026-09-25.md: the selected run's pending gates and execution-bound asks by exact identity, approve/reject through the owner routes with CSRF, the panel re-read after each decision; a gate approved and one rejected in real Chromium); the run-creation surface and T049 stay open. 2026-09-25 (tool gate): the run view and approval screen follow execution-bound waits (`awaiting_execution`/`rejected_execution` in the runs projection and `runtime.mjs`, an `expired` state and each ask's time limit on the screen) and a gated tool call is approved, rejected and retried through the screen in real Chromium (evidence/tool-gate-scheduler-t087-2026-09-25.md); the run-creation surface and T049 stay open. 2026-09-26 (run creation): the work page's `실행 시작` section (`app/static/run-start.mjs`) over `GET /api/v1/run-environments/{work_id}` (`runs.environments`: each prepared environment version bound back through its head, the owner's design approval, the stored graph record whose digest equals the design hash, and the work model's revision of this work; unresolvable versions counted, never offered) shows exactly what a run uses (environment version, graph, work revision, bindings, grants, qualification, budget, consent scope), records the consent through `run-consents-v1`, starts through `runs-v1` and links to `observe.html#run=…`; without a prepared environment it states the absence (no approved design: no critic can be qualified here) and offers no start. Real Chrome (`browser-run-start-t048.test.mjs`): the production state (absence, preparation refused) and a SIMULATED-qualification environment prepared from the page, consented, started, observed, gate approved and completed (evidence/run-start-t048-2026-09-26.md). Still open: production can never prepare an environment (T077), no production path registers a design request, and T049. 2026-09-26 (UI phase 3, not a tick): the read-only run trace `GET|HEAD /api/v1/runs/{run_id}/trace` (`run-trace-v1`, browser session only) returns what the run recorded (visits and attempts with their own inputs, outputs, tool calls, reservations, errors and journal; the executor's model calls with tokens; hand-offs, approvals, final results, where it stopped), unrecorded values as `not_recorded`; `GET /api/v1/events` takes `run_id`/`work_id`; the observe page became the run detail (final result first, graph | timeline sharing one selection, visit/attempt pickers with past attempts kept to their own facts, per-node outputs, in-place `내 버전`) — real Chrome in `browser-run-detail.test.mjs` (evidence/run-trace-ui-2026-09-26.md). Owner screenshot review, per-call model cost on the Claude executor path and the items above stay open. 2026-09-26 (recorded usage, not a tick): the product now records what the provider reports — the ledger journals a transport's `ProviderUsageReport` (input/output/cache tokens, observed model, message/request ids when held) once with an attempt's accepted result (`provider_usage`; the semantic provider transport reports it), and the Claude executor records each call's cost with its basis (`subscription_mode`; a `reserved_ceiling` estimate from the recorded tokens at the operator's `DEEPTWIN_LIVE_CEILING_*` rates in the budget's currency; else `not_recorded`); the trace reads them and keeps `attempt_tokens`/`model_cost` only where still unrecorded; no schema change (evidence/run-trace-ui-2026-09-26.md §"Recorded usage"). Still open: a settled cost on the executor path (it never settles against the budget book), ceiling rates in production (none configured by default), hand-off receipts, owner screenshot review. 2026-09-26 (UI phase 4, not a tick): the owner's process feedback on a run landed — an optional mark (`ok`/`needs_attention`) and an optional memo (≤4,000 characters), on the run or one node's exact visit and attempt, as immutable `process_feedback` revisions through `run-feedback-v1` (`GET|HEAD|POST /api/v1/runs/{run_id}/feedback`, CSRF owner command, `feedback.recorded` in the same transaction), attached to the run trace; never an alternative. The run detail shows it ("이 결과 전체" in the final result, the selected attempt in the selection panel, graph/timeline markers, a header line) and "차이 살펴보기" opens in place under the artifact with the related run segment and the inquiry inside (docs/ui/2026-09-26-product-ux-redesign.md §13, evidence/process-feedback-2026-09-26.md). Owner screenshot review and the items above stay open. 2026-09-26 (UI phase 5, not a tick): `observe.html` without `#run=` shows the run list as a table (업무 · 환경 버전 · 상태 · 시작 · 걸린 시간 · 비용 · 사람 대기, each row read from that run's `run-trace-v1`, unrecorded values "—"/"미확인" with a note; the environment version is carried by no route and stays "—"); rows open the run detail, which keeps a compact "다른 실행" switch and a link back to the list; the work page's ④ 준비·시작 step starts runs with "업무 시작" and lists this work's runs (`app/static/run-summaries.mjs`, docs/ui/2026-09-26-product-ux-redesign.md §14). No route added. Owner screenshot review and the items above stay open.
- [x] T049 [US3] Run actual controlled browser/PDF/table/image producer→consumer E2E, then finite authorized Claude/Codex paths, in app/tests/browser-runtime.test.mjs and specs/001-autonomous-release/evidence/us3.md (SC-001/004). 2026-09-25: the controlled E2E and the Claude path landed (evidence/us3.md): in real Chromium over real processes (fetch 20104, browser 20105 in its own netns, app 20102, document worker), `page-reader` (browser_read) and `page-shooter` (browser_screenshot) each make one real attempt, `documents` renders PDF/CSV/PNG with the document adapter, a join and the owner's gate feed a consumer that reads every artifact whole (the PDF via the document worker) and reports their exact digests; the observe page shows graph states, one row per attempt, the PDF page, CSV table, both images and the page text; a SIGKILL restart keeps the trace identical; a timed-out read, the owner's recovery (attempt 2, a distinct row) and a cancel. Needed: per-node transports in `NodeAttemptDispatcher`, browser outputs listed as run artifacts, and a fix — recovery after a restart replayed attempt 1's reserve under the dead process's owner (`CommandConflict`). One live Claude run (model role consumed the produced CSV; 125/18 tokens, ≈$0.001, under the $2 cap) through `ClaudeRunExecutor(producers=...)`. Open: the Codex path (no Codex subscription here; not simulated), T042 multimodal turns.
  2026-09-25 closed: the controlled browser/PDF/table/image producer→consumer E2E (real workers, persisted grant,
  restart/recovery/cancel) and the finite authorized Claude path ran (evidence/us3.md). The Codex path is
  not run here: no Codex subscription exists in this environment, and on 2026-09-25 the owner said they will
  verify the Codex side separately with Codex. It is recorded as owner-delegated, not as passed.
...
- [ ] T066 [US6] Implement paired execution/round/artifact comparison, actual stop explanations and human version approval/rollback UI in app/static/experiments.mjs and app/static/versions.mjs (UX-AC06).
  ...
  2026-09-26 note (UI phase 5, not a tick): `versions.html` is split into tabs 운영 버전 · 후보 · 실험 ·
  승인·적용·롤백 (counts in the tab names; gates in 후보, approve/reject/defer/apply and rollback in 승인·적용·롤백,
  experiments with paired rounds and boundaries in 실험) with stated empty states ("아직 운영 버전이 없습니다. 새
  환경에서는 정상입니다."); behaviour unchanged (docs/ui/2026-09-26-product-ux-redesign.md §14.4). Still open as above.
...
- [x] T073 [US7] Implement anywhere-accessible log/export/backup/retention GUI with actual included-content preview and consent in app/static/records.mjs and app/static/settings.mjs; no mandatory final export step (UX-AC08).
  ...
  2026-09-25 reading erasure (T023 gap): the deletion preview lists every reading of each
  chosen original (text read on the owner's command), readings sharing that text, and the
  work-model drafts made from them, and states what backups made after (neither) and before
  (both, owner-held, never rewritten) hold; consent tombstones each reading's text blob per
  content address and removes the bytes after commit; reading records stay as metadata
  answering `deleted` (text route 410); a later backup lacks the text (canary scans of DB,
  WAL, blob dirs and backup archive) — evidence/source-deletion-t073-2026-09-23.md
  (2026-09-25). Kept, and said so in the preview: work-model drafts (the model's own wording).
...
- [ ] T078 After T087's extension UI and the frozen T081 candidate exist, perform final visual/
  ...
  (UX-AC01–11).
  2026-09-25 note (not a tick): a first `Settings > Extensions` screen exists (evidence/extensions-ui-2026-09-25.md).
  It shows source/license/port/qualification/failure from the routes that exist, and marks binding, five-field slot
  ...
  2026-09-26 note (not a tick): UI redesign phase 3 landed (docs/ui/2026-09-26-product-ux-redesign.md §12,
  evidence/run-trace-ui-2026-09-26.md): the run detail screen over the read-only run trace, screenshots at 1280px,
  390px and dark 1280px. Phases 4–6 and this task's own keyboard/screen-reader/360px/IME checks remain open; no owner
  review of the screenshots is recorded yet.
  2026-09-26 note (not a tick): UI redesign phase 4 landed (docs/ui/2026-09-26-product-ux-redesign.md §13,
  evidence/process-feedback-2026-09-26.md): process feedback (괜찮음 / 확인 필요 toggles with aria-pressed, an optional
  memo, explicit save and clear) on the whole result and on the selected attempt, graph and timeline markers that
  spell their meaning beside the glyph, "차이 살펴보기" in place under the artifact, and the phase-3 clean-ups
  (artifact index moved to the records page, the approvals card folded when nothing is pending, local-time
  deadlines, honest run-stop chips, no "다음 기록 보기" at the log's end). Screenshots at 1280px, 390px and dark
  1280px. Phases 5–6 and this task's own keyboard/screen-reader/360px/IME checks remain open; no owner review of the
  screenshots is recorded yet.
  2026-09-26 note (not a tick): UI redesign phase 5 landed (docs/ui/2026-09-26-product-ux-redesign.md §14): the work
  page as steps (reached steps and one next action, folding summaries with aria-expanded toggles, a work menu and
  this work's runs beside it), the run list table (cards with labels below 760px), records filters by work, run,
  kind and period, the versions tabs, the difference view reachable again after a reload and the feedback history.
  Screenshots at 1280px, 390px and dark 1280px. Phase 6 and this task's own keyboard/screen-reader/360px/IME checks
  remain open; no owner review of the screenshots is recorded yet.
  2026-09-26 note (not a tick): UI redesign phase 6 landed (docs/ui/2026-09-26-product-ux-redesign.md §15,
  evidence/ui-accessibility-2026-09-26.md): an in-house accessibility audit in app/tests/browser-a11y.test.mjs
  (axe-core was not available offline) over every shell page and every settings panel in both themes — names,
  labels, images, headings, landmarks, IDREFs, duplicate ids, rendered and token contrast (AA), a visible focus
  ring on every tab stop, reflow at 320px and at 1280px under 200% zoom, reduced motion — and six keyboard-only
  journeys (a work saved; a run opened from the list, its graph walked, its attempt and detail tabs switched; 내 버전
  and 차이 살펴보기 opened in place and closed with focus returned; 확인 필요 saved; the 390px drawer trapping and
  returning focus; every settings panel), with polite live regions checked for repeats. Fixed: the light accent's
  contrast, field edges, the context bar outside any landmark, duplicate landmark names, names on generic elements,
  a skipped heading level, focus lost after an attempt switch, a feedback save and "실행 전체 보기", the drawer's
  focus leak, repeated live-region text, detail tabs hidden at 320px. Also: records log first on narrow screens
  with two-column filters and a hash-only change re-read, the run trace's recorded `run_mode` behind the context
  bar's mode badge, shortened digests, concise backup lines and `설정 > 서비스 클라이언트`. Not done here: a real
  screen-reader user test, IME and three-mode checks, the T087 extension UI checks and the frozen T081 candidate;
  no owner review of the screenshots is recorded yet.
