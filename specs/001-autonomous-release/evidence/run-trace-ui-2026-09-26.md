...
- The Claude executor path records no per-call cost. The fixture's model call therefore shows
  "미확인" with basis `not_recorded`. The estimate and settled bases are exercised only by the
  server test with an API-priced reservation.
- Ledger attempts record no token counts (`attempt_tokens`).
