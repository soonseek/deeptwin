...
## Task 24 step (c1) — installation anchor and success consumption codecs (2026-09-18)

RED retained (module absent, 14 unraised violations), GREEN after three test-side corrections (record
API shape, `_constant` type, top-level `Record` ref). Independent review ACCEPT WITH CHANGES: MUST (local
`^…$` anchoring admitted a trailing newline through `ImmutableRecord.create` → canonical `text()`),
SHOULD ×2 (export pairs content variant with parent arity; two tests isolated to their rule), NIT ×2 —
all RED-first. Covering (installation domain 30, receipt domain, exports, journal integrity/reconciliation,
import): **145 passed**; Ruff clean on the codec files, pre-existing findings unchanged. Details and hashes
in `extension-installation-domain-task24c1.md`. Full regression for this iteration follows.
Full regression (`app/tests deploy/tests`, watchdog 1500 s): **5997 passed, 2 failed, 2 skipped, 1 warning,
369 subtests in 715.19s**; the two failures were the pre-Task-22 artifact byte-pin (the envelope export is
regenerated by contract) and the global-guard test's free-form installation row — both adapted as tests
(see the evidence) and re-run green with their whole modules (170 passed); no `app/` code changed after
the full run. Hashes re-verified unchanged before commit.

## Task 24 step (c2) — v3 storage constants, consume codec, prepare-api-v3 (2026-09-18)

RED retained (`DDL_V3` absent, module absent), GREEN after one design correction (the consume input
matched two `oneOf` alternatives of the api artifact → the import-shaped input alternative covers both
routes, stated in the artifact `$comment`) and one test isolation (CHECK matrix with foreign keys off).
Independent review ACCEPT WITH CHANGES (v2 error-envelope helper, `$comment`, `match="CHECK constraint
failed"`, PRAGMA-backed nullable check) folded in. Covering: **197 passed** (v3 contracts 15, v2 exports
and contracts, storage, receipt exports, lineage contracts incl. the 68-artifact pin); Ruff clean.
