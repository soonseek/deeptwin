...
| Runtime (implemented 2026-09-26, `run-trace-v1`) | `GET\|HEAD /api/v1/runs/{id}/trace` — owner browser session only (a bearer gets `401`), no query, no body; `run-trace-v1`: `run_id`, `phase`, `graph_ref`, `graph_digest`, `work` (`work_revision_ref`, `work_id`, `revision`, `title` = first line of a `work-revision-v1` text or `not_recorded`), `budget_mode`, `run_mode` (2026-09-26 UI phase 6: the `mode` the runtime ledger froze in the run spec — `live`/`replay`/`snapshot`/`isolated-comparison` — or `not_recorded`; read, never inferred), `started_at_utc`, `ended_at_utc` (the completed/cancelled stop), `stops[]` (`reason`, `at_utc`, `duration_ms`), `totals` (`model_calls`, `tool_calls`, `attempts`, `retries`, `input_tokens`, `output_tokens`, `tokens_complete`, `cost` {`state` ∈ `settled`/`estimate`/`partial_estimate`/`unknown`, `microunits`, `currency`}), `exit` (`node_ids`, `basis` ∈ `completion_criteria`/`graph_sinks`), `final_results[]` (the exit visits' artifacts: `artifact_id`, `role`, `declared_media_type`, `size`, `sha256`, `result_ref`, `node_id`, `execution_id`, `visit_no`, `produced_by_attempt_no`), `stopped_at[]` (`node_id`, `state`), `nodes[]` (`node_id`, `kind`, `responsibility`, `state`, `visits[]`: `execution_id`, `visit_no`, `status`, `recorded_at_utc`, `result_ref`, `outputs[]`, `produced_by_attempt_no`, `inputs[]` (the parent visits recorded at dispatch: `from_node_id`, `from_execution_id`, `from_attempt_no`, `result_ref`, `artifacts[]`), `attempts[]` (`attempt_no`, `phase`, `terminal_outcome`, `reserved_at_utc`, `sent_at_utc`, `ended_at_utc`, `produced_visit_result`, this attempt's own `result_ref`/`outputs[]`, `error` {`outcome`, `reason_code`, `remote_terminal_observed`}, `tool_calls[]` {`tool_id`, `version`, `effect_class`, `state`, `declared_inputs[]`, `binding` (graph `binding_id`, `definition_ref`, `grant_ref`, gate), `approval_ref`, `requested_at_utc`, `settled_at_utc`, `result_ref`, `result_artifacts[]`}, `tokens` {`input`, `output`, `cache_creation_input`, `cache_read_input`} and `observed_model`, `provider_message_id`, `request_id` (2026-09-26: what the provider reported to this attempt's transport, journaled by the ledger as `provider_usage` with the accepted result; `not_recorded` when the transport reported none), `budget_reservation` (reserved and settled counters, `api_microunits`), `cost` {`state`, `basis` ∈ `budget_settlement` (settled)/`reserved_ceiling` (estimate, `method: reservation` = the reserved amount)/`subscription_mode`/`not_recorded`}, `journal[]` transitions), `model_calls[]` (Claude executor records: `provider`, `model_label`, `observed_model`, `provider_message_id`, `request_id`, `effort`, `max_output_tokens`, `state`, `stop_reason`, `error`, `tokens` {`input`, `output`, `cache_creation_input`, `cache_read_input`}, `started_at_utc`, `ended_at_utc`, `inputs[]`, `output_ref`, `cost` (2026-09-26: recorded by the executor with the call — `subscription_mode` under a subscription budget; `reserved_ceiling` with `state: estimate`, `method: recorded_tokens_at_ceiling_rates`, `microunits`, `currency` and the `rates` used when the budget is API-priced and the deployment configured ceiling rates in that currency; else `not_recorded`; a call recorded before that reads `subscription_mode` or `not_recorded` from the run's budget, never priced at read), `reasoning: not_stored`)), `handoffs[]` (`from_*`, `to_node_id`, `to_execution_id`, `to_attempt_nos`, `artifacts[]`, `designed_edge_ids`, `receipt: not_recorded`), `approvals` (`gates[]`, `executions[]` with state and times), `timeline[]` (visits, attempts and model calls in time order), `gaps[]` (`category`, `reason`; since 2026-09-26 `attempt_tokens` only for a sent model attempt without reported tokens, `model_cost` only for a model call whose cost basis is `not_recorded`, `reasoning` for any model call or sent model attempt), `links` | read-only: no execution, no provider or tool call. Assembled only from the manifest, the runtime ledger (executions, attempts, result observations, journal, ToolCall, bound checkpoint rows), the budget reservations, the Claude executor's call records and the approval records; an unrecorded value is `not_recorded`, never computed. No hidden reasoning, credential material, request body or raw provider payload. `GET /api/v1/events` (and `/stream`, `/{event_type}`) take one optional `run_id` or `work_id`: events whose own record names the subject (a `work_revision` reference; the run's `run.started`/`run.stopped` by their creating-command correlation, or a run/attempt reference); the subject is part of the cursor's filter identity |
| Process feedback (implemented 2026-09-26, `run-feedback-v1`) | `GET\|HEAD /api/v1/runs/{id}/feedback` — owner browser session only (a bearer gets `401`), no query, no body; `process-feedback-list-v1`: `run_id`, `memo_max_chars` (4000), `marks`, `current` {`run`, `steps[]`: the latest revision of every target, cleared ones with `state: cleared`}, `history[]` (every revision). `POST /api/v1/runs/{id}/feedback` — `process-feedback-command-v1`: `command_id`, `action` ∈ `set`/`clear`, `target` (`{scope: run}` or `{scope: step, node_id, visit_no, attempt_no}`; `attempt_no` null only for a visit without ledger attempts), `expected_revision`, `mark` ∈ `ok`/`needs_attention`/null, `memo` (null or 1–4,000 characters of plain text); answer `201` {`recorded`, `replayed`, `current`}. `run-trace-v1` additively carries `feedback` {`run`, `steps`} and `feedback` on each attempt and visit | a set needs a mark or a memo, either alone (no explanation is ever required, FR-016); a clear is a new revision, never a deletion; a change names the revision it was made from (`409` when stale); CSRF-verified owner command with one command id (exact replay returns the same revision, another body under the id is `409`); the immutable `process_feedback` record and `feedback.recorded` (scope, mark, memo present, cleared, revision — never the memo text) in one transaction; the target must exist in the run's trace (`404`); never an alternative and never counted as one (UX-AC05): refused as memory provenance, change-candidate evidence and knowledge; exploration may read it only as an owner-supplied observation |
| Design and own versions (additive fields, implemented 2026-09-26, UI phase 5) | `GET /api/v1/design-requests` items add `work_id`, `work_revision`; `GET /api/v1/design-requests/{id}` adds `request.work_revision_ref` (the work revision of the request's confirmed work model). `GET /api/v1/runs/{id}/artifacts/{artifact_id}/drafts` and `…/drafts/{draft_id}` add, per draft, `frozen_alternatives[]` (`alternative_id`, `revision`, `coverage`, `frozen_at_utc`) beside `frozen_revisions` | no new route or mutation; a freeze still seals one alternative per command, so a screen reopens an existing alternative's difference instead of freezing the same revision again |
