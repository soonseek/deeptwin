...
| `tool-port-v1` | `describe_tools`, `invoke_tool`, `status`, `cancel`; exact registered tool/version and validated arguments/results | the declared effect class (extension-ports.md `effect-class`: none, read, write_reversible, external_reversible, external_irreversible, instance_critical_secret, instance_critical_storage; the external family requires an explicit approval) and ToolDefinition replay policy are authoritative; unknown external outcome is held; produced files are sealed artifact refs |
