...
- The consent line's first link landed: `run-consents-v1` seals the owner's `run_consent` over
  the exact records a run names, with the owner's decided event; the run route accepts it
  (`evidence/run-consent-record-t048.md`). Next: the run route verifies that its consent names
  exactly its inputs; the design line's `environment` record; run creation from the intake
  page. No deployment, model activation or browser journey is claimed.
