...
| Actual graph generation, criticism, selection and approval | 15 | 0.07 | 1.05 | Versioned lens registry is implemented, and the work-model confirmation, design-generation boundary and graph schema/compiler/projection slices pass 122 focused tests with external-authority binding and structural duplicate rejection; a fresh independent adversarial re-audit (2026-09-12) rejected them with 2 P1 + 3 P2 + 1 P3 and all five actionable findings are now fixed with per-finding regression tests and probe-confirmed closure (evidence/graph-compiler-reaudit-t027t030.md, design-generation-graph-t030-status.md); the F6 join tie-break axis and the live generation path remain open, so no fraction credit is taken yet; prototypes and offline critic contracts exist, but no full live graph path |
