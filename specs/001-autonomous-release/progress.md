...
- [ ] T018 now means authenticated isolated web-worker IPC and staged topology, not a native launcher.
  ...
  evidence/worker-ipc-foundation-t018f1.md (2026-09-12). The bounded digest/chunk/receiver-credit artifact stream state machine (offer/credit/chunk/end/accept, cancellation, owned scratch) is also implemented transport-agnostically with 26 unit tests; it runs over a real authenticated broker FrameCodec (FrameCodecTransport, 4 socketpair tests) and is now wired into the coordinator for the input direction: `WorkerCoordinator.exchange` accepts declared `artifact_inputs`, validates identity/batch shape before permit consumption, streams the batch after the request frame on a duplex route-declared message type, and fails closed with `outcome_unknown` past that point (7 real-socketpair tests; full regression 2,971 pass). The worker→control output direction, dispatch-layer plumbing, CAS-backed sources/sinks and the Linux/root UDS canary run are still open (the 2026-09-12 Docker attempt was blocked by a host content-store fault) — see evidence/artifact-stream-t018f1.md (2026-09-12). T018 remains open for
