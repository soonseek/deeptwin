...
- The consent line's first link landed: `run-consents-v1` seals the owner's `run_consent` over
  ...
  one consent per run (`evidence/run-consent-verification-t048.md`). Next: the design line's
  `environment` record; run creation from the intake page. No deployment, model activation or
