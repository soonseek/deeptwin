...
| LangGraph scheduling adapter (T040 first slice) | app/runtime/scheduler.py: closed code-owned handler registry, deterministic ledger-reconciled node visits via command replay, sealed router activations routed by Command, opaque saver cursors, no streaming, sanitized node failures; 8 real StateGraph+SQLite tests incl. restart-after-failure without re-running durable visits. human_gate/bounded_loop/retry/budget/semantic admission remain open |
