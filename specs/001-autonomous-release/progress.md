...
| Actual graph generation, criticism, selection and approval | 15 | 0.07 | 1.05 | Versioned lens registry is implemented, and the work-model confirmation, design-generation boundary and graph schema/compiler/projection slices pass 122 focused tests with external-authority binding and structural duplicate rejection; a fresh independent adversarial re-audit (2026-09-12) rejected them with 2 P1 + 3 P2 + 1 P3 and all five actionable findings are now fixed with per-finding regression tests and probe-confirmed closure (evidence/graph-compiler-reaudit-t027t030.md, design-generation-graph-t030-status.md); F6 is also closed (tie-break branch-signature order axis, 2026-09-12); the live generation path is partially driven: app/services/design_live.py (2026-09-13) renders the exact bounded candidate prompt, runs one untrusted model_turn boundary, mints the generation-call record itself and admits graphs only through the compiler authority (7 offline tests over a scripted model); no provider adapter is bound and no fraction credit is taken until the criticized/selected/approved live path exists; prototypes and offline critic contracts exist, but no full live graph path |
| Actual multi-agent runtime, framework tools and artifacts | 20 | 0.24 | 4.8 | Durable run/attempt, budget and root-command substrate plus a historical pure extension-contract checkpoint exist; ADR-014 shows that generic messages, memory-only lifecycle and source-tree helpers are not a completed SPI/SDK. T018-A/B/B2 contribute independently reviewed transport and bounded HTTP→coordinator handoff scope, and the bounded artifact byte route is now complete and adversarially audited end to end (stream state machine, FrameCodec carriage, both coordinator directions, CAS-bound endpoints; 2026-09-12/13), while semantic ports integration, durable extension state/staging, graph scheduler and real workers/tools remain |
...
| Total | 100 | — | 42.55 | User-facing estimate: roughly 40–45% (42.6 unrounded); the design gate is closed while T018/T025/T087 and complete journey gates stay open. 2026-09-13 recalibration: the growth package moved 0.00→0.30 because the audited offline US4–US6 engine chain is genuinely new scope completion, while its storage/UI/E2E/live majority stays open |
