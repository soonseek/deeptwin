...
- The consent line's first link landed: `run-consents-v1` seals the owner's `run_consent` over
  ...
  The design line's `environment` record landed with it (`evidence/environment-record-t048.md`):
  all five records a run names now have producers. Next: run creation from the intake page, and
  the design arc's own production path (its approval still needs an accepted candidate and a
  fold-issued passed verdict). No deployment, model activation or browser journey is claimed.
...
| T048: the `environment` entity a run names | `design_store.persist_environment_record` seals one immutable `environment` record per prepared version — cut from the exact head record, carrying only the preparation that head holds, only ever for a `prepared` version, idempotent under the same headers and losing the compare-and-swap to a second writer's different design; the run route resolves it and refuses the chained head in that slot | 2026-09-23 | independent review ACCEPT WITH CHANGES (MUST: the head's content was never checked) folded in RED-first; environment records 6, design/consent/run neighbours 69; full regression **9,356 passed / 0 failed / 2 skipped** | evidence/environment-record-t048.md |
