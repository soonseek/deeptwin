...
목표로 개발 중이다. 저장소는 Apache-2.0으로 라이선스된다(`LICENSE`, `NOTICE`, `REUSE.toml`). 다만 배포 이미지·서명·
두 호스트 검증(T081–T083)이 남아 있어 아직 공개 릴리스 후보 상태는 아니다. 릴리스에서 지원할 **공식 최종사용자 제품·관제면은 브라우저
