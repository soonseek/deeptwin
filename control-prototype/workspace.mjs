...
import { artifact, artifactLabel, button, e, graph, list } from './primitives.mjs';
...
function designWorkspace(state) {
  ...
  return '<section class="work-model"><h2>업무 모델</h2>'
    ...
    + `<section class="design-comparison" aria-label="같은 관점에서 설계 비교">${DESIGNS.map(design => `<div class="design-option" data-design-option="${e(design.id)}">`
      + `<h3>${e(design.label)}</h3><p class="design-contract">${e(design.contracts[state.focus])}</p>`
      + button('이 설계 살펴보기', 'design', design.id, state.design === design.id) + '</div>').join('')}</section>`
    + `<div class="candidates">${DESIGNS.map(design => `<section class="candidate" data-design="${e(design.id)}"${state.design === design.id ? '' : ' hidden'}>`
      + designDetails(design, state.focus, 'candidate') + '</section>').join('')}</div>`
    + (selected.id === REVISED_DESIGN.id
      ? '<section class="merged-design"><h2>병합 예시의 전체 구조 · 별도 버전 검토</h2>'
        + designDetails(selected, state.focus, 'selected') + '</section>' : '')
    + `<section class="selected-design" data-selected-design="${e(selected.id)}"><h2>선택한 설계의 요청·승인 대상 기록</h2>`
    + facts([['선택한 설계', selected.label], ['정확한 설계 버전', selected.version], ['검토 상태', selected.review]])
...
function artifactChoices(ids, selectedId) {
  return '<div class="artifact-choices">' + ids.map(id => button(`${artifactLabel(ARTIFACTS[id])} · ${ARTIFACTS[id].type.toUpperCase()}`, 'artifact', id, selectedId === id)).join('') + '</div>';
}

function fullFiles(ids) {
  return ids.map(id => `<details class="file-detail"><summary>${e(ARTIFACTS[id].label)} · ${e(id)}</summary>${artifact(id)}</details>`).join('');
}

function inspection(run, node, attempt, { historyAction, consumerAction, prefix = '', full = false, selectedArtifact }) {
  if (!node) return '<p>노드 미확인</p>';
  const history = run.attempts.filter(item => item.node === node.id);
  const heading = `<h3>${e(node.label)}</h3>`;
  ...
  const inputs = full ? fullFiles(attempt.inputs) : artifactChoices(attempt.inputs, selectedArtifact);
  const outputs = attempt.outputs.length ? (full ? fullFiles(attempt.outputs) : artifactChoices(attempt.outputs, selectedArtifact)) : artifact(null);
  const consumers = attempt.consumers.length
    ? `<ul class="consumers">${attempt.consumers.map(consumer => `<li><span>${e(consumer.artifact)} → </span>`
      + button(`${run.nodes.find(item => item.id === run.attempts.find(item => item.id === consumer.attempt)?.node)?.label ?? '수신 시도'} · ${consumer.attempt}`, consumerAction, `${prefix}${consumer.attempt}`) + '</li>').join('')}</ul>`
    : '<p>기록된 실제 수신자 없음</p>';
  return `<div class="attempt-inspector" data-inspected-attempt="${e(attempt.id)}">${heading}${controls}`
    + `<h4>이 시도의 모든 출력</h4>${outputs}`
    + '<details class="inspector-details"><summary>입력·전달·사건의 상세 기록</summary>'
    + `<p class="version">${e(runLabel(run))} · ${e(node.id)}</p><p class="attempt-status">${e(attempt.id)} · ${e(attemptLabel(attempt))}</p>`
    + `<h4>정확한 입력 버전</h4>${inputs || '<p>입력 없음</p>'}`
    + `<h4>실제 후속 수신자</h4>${consumers}<details class="events"><summary>이 시도의 사건 기록</summary>${list(attempt.events)}</details>`
    + '<p class="trace-boundary">계획된 연결과 실제 산출물 전달은 다릅니다. 실제 전달 기록도 적절한 사용이나 인과관계의 증명이 아닙니다.</p></details></div>';
}
...
function runEditor(state) {
  ...
  const run = RUNS[selected.run];
  const attempt = run.attempts.find(item => item.id === selected.attempt);
  const isOutput = attempt.outputs.includes(selected.artifact);
  const producer = run.attempts.find(item => item.outputs.includes(selected.artifact));
  const role = id => run.nodes.find(item => item.id === id)?.label ?? id;
  const provenance = isOutput ? `이 수행이 만든 출력 · ${role(attempt.node)}`
    : `이 수행이 받은 입력 · ${producer ? role(producer.node) : '생성 수행 기록 없음'} → ${role(attempt.node)}`;
  const key = draftKey(state);
  const hasDraft = (state.drafts[key] ?? '').length > 0;
  return '<section class="alternative-editor"><div class="original-region"><h3>선택한 원본</h3>'
    + `<p class="artifact-provenance" data-role="${isOutput ? 'output' : 'input'}">${e(provenance)}</p>`
    + '<details class="origin-preview" open><summary>원본 미리보기·전체 또는 부분 선택</summary>'
    + artifact(selected.artifact, { selectable: true, scope: selected.scope })
    + '</details>' + button('원본 넓게 보기', 'overlay', 'artifact') + '</div>'
    + '<div class="user-version-region"><h3>같은 조건에서 내가 만든 버전</h3><p>수정 지시가 아니라, 내가 직접 만든 결과를 남깁니다.</p>'
    ...
    + button('이 입력의 개선 영역 열기', 'currentGrowth') + '</div></section>';
}
...
function runWorkspace(state) {
  ...
  return `<details class="run-picker"><summary>실행 선택 · ${e(runLabel(run))}</summary><nav aria-label="합성 실행 선택">`
    ...
    + `<details class="run-context"><summary>${e(sourceSummary(state))}</summary>${sourceContext(state)}</details>`
    + '<div class="run-toolbar">' + button('선택한 산출물 보기', 'inspect')
    + (selected.artifact ? button('내 버전 작성', 'compose') : '') + '</div>'
    + `<div class="run-layout"><details class="run-graph" id="run-graph-target" tabindex="-1"${state.mode === 'graph' ? ' open' : ''}><summary>실행 환경의 전체 관계도</summary>`
    + '<p class="graph-help">노드를 선택해 결과를 확인하세요. 넓은 관계도는 가로로 탐색할 수 있습니다.</p>' + graph(run, [selected.node]) + '</details>'
    + '<section class="selection-workspace" id="selection-workspace" tabindex="-1"><div class="selection-heading"><h2>선택한 작업의 산출물</h2>'
    + button('관계도로 돌아가기', 'graphBack') + '</div>'
    + `<div class="run-columns">${inspection(run, node, attempt, { historyAction: 'attempt', consumerAction: 'consumer', selectedArtifact: selected.artifact })}${runEditor(state)}</div></section></div></section>`;
}

function freshGrowth(state) {
  const selected = context(state);
  const file = ARTIFACTS[selected.artifact];
  const excerpt = selected.scope.kind === 'region' ? file?.regions.find(region => region.id === selected.scope.id)?.text
    : selected.scope.kind === 'text' ? file?.body.slice(selected.scope.start, selected.scope.end) : null;
  return '<section class="fresh-growth"><h2>현재 입력의 개선 영역</h2>'
    + '<p class="qualification">분석 결과 없음 · 실제 탐구·실험 엔진 미연결</p>'
    + `<details class="source-context"><summary>${e(sourceSummary(state))}</summary>${sourceContext(state)}</details>`
    + '<div class="artifact-pair current-pair"><section><h3>선택한 원본</h3>'
    + (excerpt == null ? '' : `<p>비교 대상으로 선택한 부분</p><blockquote>${e(excerpt)}</blockquote>`)
    + artifact(selected.artifact) + '</section><section><h3>현재 맥락의 자기 버전</h3>'
    + `<blockquote class="user-version-content">${e(state.drafts[draftKey(state)] || '현재 맥락에 작성된 자기 버전 없음')}</blockquote></section></div>`
...
function hypotheses(difference) {
  const labels = { support: '지지하는 관찰', counter: '반대 설명', unknown: '아직 모르는 것', newEvidence: '새 근거' };
  return `<div class="hypotheses">${difference.hypotheses.map(hypothesis => `<section data-hypothesis="${e(hypothesis.id)}">`
    + `<h3>${hypothesis.kind === 'system' ? '시스템 전달에 관한 설명' : '자기 버전의 판단에 관한 설명'}</h3><p class="hypothesis-claim">${e(hypothesis.claim)}</p>`
    + '<p class="hypothesis-status">미확정 · 확인할 설명이며 결론이 아닙니다.</p>'
    + `<p class="hypothesis-probe"><strong>다음에 확인할 증거</strong> · ${e(hypothesis.probe)}</p>`
    + '<details class="hypothesis-evidence"><summary>지지·반대 관찰과 아직 없는 근거</summary>'
    + facts(Object.entries(labels).map(([key, label]) => [label, hypothesis[key]])) + '</details></section>').join('')}</div>`;
}
...
function fixedGrowth(state) {
  ...
  return '<section class="fixed-example"><h2>사전 구성된 합성 개선 사례</h2>'
    ...
    + `<div class="difference-summary"><h3>${e(difference.label)}</h3>${facts([['관찰된 원본', difference.original], ['관찰된 자기 버전', difference.alternative]])}</div>`
    + hypotheses(difference)
    + '<details class="source-pair"><summary>고정 예시의 전체 원본·부분 자기 버전</summary>'
    + `<div class="artifact-pair"><section><h3>고정 예시의 원본</h3>${artifact(CASE.original)}</section>`
    + `<section><h3>고정 예시의 부분 자기 버전</h3>${artifact(CASE.alternative)}</section></div></details>`
    + '<details class="investigation-trace"><summary>관련 원 실행의 전체 관계도·수행 이력</summary>'
    + `<h3>이 차이와 관련된 원 실행</h3><p>${e(origin.id)} · 노드를 열면 이 고정 예시의 정확한 수행 이력을 볼 수 있습니다. 현재 작성 중인 원본 선택은 바뀌지 않습니다.</p>`
    + graph(origin, difference.nodes, 'sampleNode') + button('합성 탐구 감사 기록', 'overlay', 'audit') + '</details>'
    ...
    + `<div class="round-result"><h3>${e(round.candidate)}</h3><p>${e(round.result)}</p><p>${e(round.limits)}</p></div>`
    ...
    + button('승인 대상 확인', 'overlay', 'approval')
    + '<details class="paired-details"><summary>기준·후보의 전체 관계도와 정확한 수행 기록</summary>'
    + `<div class="paired-runs">${pairedRun(state, round, 'baseline')}${pairedRun(state, round, 'candidate')}</div></details></section>`;
}
...
export function render(state) {
  ...
  return `<div class="shell" data-area="${e(state.area)}" data-mode="${e(state.mode)}"><aside class="rail"><h1>DeepTwin</h1><p>멀티에이전트 관제</p>`
    ...
    + `</nav><div class="mode-layout">${conversation}<div class="mode-content">${content}</div></div><footer class="storage-footer"><p id="storage-status" role="status">아직 보관 상태를 확인하지 않음</p>`
...
function auditPreview(state) {
  const headings = { generation: '설계 생성·병합 기록', critic: '별도 검토 기록', diagnosis: '차이 해석·탐구 기록' };
  const selected = state.area === 'design' ? facts([['선택한 설계', chosenDesign(state).label], ['정확한 버전', chosenDesign(state).version]])
    : state.area === 'growth' && state.sample ? facts([['고정 사례', CASE.id], ['선택한 차이', state.difference], ['비교 회차', state.round], ['후보', chosenRound(state).candidate]])
      : sourceContext(state);
  return '<h2>합성 감사 기록</h2><section class="audit-context"><h3>지금 살펴보는 대상</h3>' + selected
    + '<p>아래는 전체 합성 기록입니다. 선택한 대상과 특정 내부 경로의 직접적인 연결 근거가 없는 경우, 그 경로의 결과라고 단정하지 않습니다.</p></section>'
    + '<p>내부 명칭을 포함한 작성 예시입니다. 철학적 원전 검증이나 판단 오류의 독립성 증명이 아닙니다. 사후 해석은 원 실행의 입력에 포함되지 않습니다.</p>'
    + Object.entries(headings).map(([key, heading]) => `<details class="audit-group"><summary>${e(heading)}</summary>${list(CASE.audit[key])}</details>`).join('');
}
    ...
    : id === 'approval' ? approvalPreview(state)
      : id === 'connection' ? connectionPreview()
        : id === 'logs' ? logsPreview(state)
          : id === 'audit' ? auditPreview(state) : '<h2>보기 미확인</h2>';
